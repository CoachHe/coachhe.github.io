...
需要注意的是， `WatermarksStrategy` 是一个接口，无法直接 `new`，因此若是要自己实现这个接口还是比较麻烦，因此对于一般应用我们不会自己去实现这个接口，`flink` 的 `WatermarkStrategy` 接口中自带了几个抽象方法，可以用来获取 `flink` 内置的类。
...
并且生成 `WatermarkStrategy` 之后还需要实现一个 `.withTimestampsAssigner` 方法，这个方法是为了
