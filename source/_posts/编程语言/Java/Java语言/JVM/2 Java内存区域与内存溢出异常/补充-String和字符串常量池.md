...
要了解字符串常量池我们需要先知道运行时常量池，这里会在 [[3 运行时数据区域]]中进行详细介绍，这里我们就不展开了，主要来看字符串常量池。
...
进行分析，我们可以看到，首先从字符串常量池的第 14 个位置获取字符串常量 `mango` ，并将其放入本地变量表的第一个位置，注意，这里第 0 个位置是默认的输入参数 `args`，因为是静态方法所以没有 `this` 变量，具体的可以看 JVM 内容 [[2. 运行时栈帧结构]]。
