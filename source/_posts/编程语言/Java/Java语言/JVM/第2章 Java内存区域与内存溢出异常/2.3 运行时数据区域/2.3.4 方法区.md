...
可以看到，用 static final 修饰的全局常量，在编译阶段就确认了变量值 2，而没有用 final 修饰的则没有，在类加载的准备和初始化阶段才会进行赋值[[3. 类加载的过程]]。
...
1. 该类的所有实例都已经被回收了。也就是 Java 堆中不存在该类的
2. 加载该类的类加载器已经被回收
3. 该类对应的 `java.lang.Class` 对象没有在任何地方被引用，无法在任何地方通过反射访问该类的对象。

在大量使用反射、动态代理、CGLib 等字节码框架，动态生成 JSP 以及 OSGi 这类频繁自定义类加载器的场景中，通常都需要 Java 虚拟机具备类型卸载的能力，以保证不会对方法区造成过大的内存ya'li
